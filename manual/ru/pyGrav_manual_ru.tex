% Предустановленные пакеты:
%   texlive-full
% Команда для сборки:
%   latexmk -pdf -bibtex -latexoption="-file-line-error -synctex=1 -interaction=nonstopmode -shell-escape" $fullname

\documentclass[10pt,a4paper,twoside]{extreport}
\usepackage[utf8]{inputenc}
\usepackage[russian]{babel}
\usepackage[OT1]{fontenc}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{makeidx}
\usepackage{graphicx}
\usepackage{hyperref}
\usepackage{listings}
\usepackage{titlesec}
\titleformat{\chapter}[hang]{\LARGE\bfseries\raggedright}{\thechapter}{0.5cm}{}[]
\usepackage[left=1.5cm,right=1.5cm,top=1.5cm,bottom=1.5cm]{geometry}
\sloppy

% \documentclass[10pt,twoside]{geodmanual}

\title{pyGrav}
\author{Basile Hector}

% \manualVersion{\ShowRepoVersion{}}

% \orgFullName{}
% \orgShortName{}
% \divName{}
% \depName{}

% \manualType{Руководство пользователя}
% \manualYear{2023}
% \manualCity{Астана}
% \def \pathtotitlepic {figures/a10.png}

\includeonly{
  chapters/overview,
  chapters/quick_start,
  chapters/pygrav_functions,
  chapters/test_case,
  chapters/code_structure,
  chapters/acquisition_protocol,
  chapters/least_square_inversion_and_error_propagation,
  chapters/installing_external_programs,
}

\newcommand{\pg}{\textbf{\textsf{pyGrav}}}

\begin{document}

\maketitle
\tableofcontents

\include{chapters/overview}
\include{chapters/quick_start}
\include{chapters/pygrav_functions}
\include{chapters/test_case}
\include{chapters/code_structure}
\include{chapters/acquisition_protocol}
\include{chapters/least_square_inversion_and_error_propagation}
\include{chapters/installing_external_programs}

% \appendix

\bibliographystyle{apalike}
\bibliography{biblio}

\end{document}
